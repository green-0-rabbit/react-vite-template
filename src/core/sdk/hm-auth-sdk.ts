...
import { errorResponse, isResolved, response } from "../../common/utils";
...
export class HMAuthSDK {
  ...
  private _isUUID = (value: string) => {
    const match = value.match(/^[0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}$/g)
    const res = match ? Boolean(match.length) : Boolean(match)
    return res
  }
  ...
  refreshToken = async (isWeb = true) => {
    ...
    try {
      const uid = this._getCookie("uid")
      this._resetValues();

      // we simulate an error on client in order to limit server call for an unauthenticated user
      if (!this._isUUID(uid)) {
        throw new HMAuthSDKError({ name: "UnauthorizedException", message: "Unauthorized", statusCode: 401 })
      }
